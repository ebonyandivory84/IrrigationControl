...
class Scheduler {
    ...
    async _tickAutomatic() {
        ...
        if (morningHandled !== todayStr && this._timeReached(now, morningTime)) {
            ...
            const result = await this._evaluateAndMaybeRun('auto-morning');
        ...
        // 2) Zwischenwässerung (nur wenn 3 Läufe/Tag aktiv, Morgen-Checkpoint für heute bereits erledigt, tagsüber vor Abend-Zeit, hitzegetriggert, 1x/Tag)
        if (maxRuns >= 3 && morningHandled === todayStr && interimHandled !== todayStr && !this._timeReached(now, eveningTime)) {
            ...
            if (hot) {
                ...
                await this._evaluateAndMaybeRun('auto-interim');
            }
        }
        ...
        if (morningSkippedForRain && eveningHandled !== todayStr && !this._timeReached(now, eveningTime)) {
            ...
            if (!weather.stale) {
                ...
                const currentOk = weather.rainCurrentMm == null || weather.rainCurrentMm < rainCurrentThreshold;
                const forecastOk = weather.rainForecastPct == null || weather.rainForecastPct < rainForecastThreshold;
        ...
        // 4) Abend-Checkpoint: Standard-Zweitlauf (maxRuns>=2) ODER Fallback-Auflösung eines offenen Regen-Skips
        if (eveningHandled !== todayStr && this._timeReached(now, eveningTime) && (maxRuns >= 2 || morningSkippedForRain)) {
            await this.adapter.setStateAsync('status.autoEveningHandledDate', todayStr, true);
            if (morningSkippedForRain) {
                const result = await this._evaluateAndMaybeRun('auto-fallback', { ignoreForecast: true });
                if (result.ran) {
                    await this.history.addEntry({
                        timestamp: new Date().toISOString(),
                ...
                await this._evaluateAndMaybeRun('auto-evening');
            }
        }
    }
    ...
    async _evaluateAndMaybeRun(trigger, opts = {}) {
